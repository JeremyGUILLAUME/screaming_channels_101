...
for target_freq in target_freqs:
    if args.profile!=0:
        ...
        for attck in range(nb_attacks):
            ...
            KeyRanks.append( [128] + [bruteforce.complexity(plaintexts, key, LOG_PROBAs[attck][i]) for i in range(len(LOG_PROBAs[attck]))] )
            
        np.save(data_directory+"LOGs/LOG_PROBAs_%s_%d_%dattacks"%(dataset_name,target_freq,nb_attacks), LOG_PROBAs)
        np.save(data_directory+"KRs/KeyRanks_%s_%d_%dattacks"%(dataset_name,target_freq,nb_attacks), KeyRanks)
        

if args.profile==0:
    LOG_PROBAs = []
    LOG_PROBAs.append( np.load(data_directory+"LOGs/LOG_PROBAs_%s_%d_%dattacks.npy"%(dataset_name,target_freqs[0],nb_attacks))   )
    LOG_PROBAs.append( np.load(data_directory+"LOGs/LOG_PROBAs_%s_%d_%dattacks.npy"%(dataset_name,target_freqs[1],nb_attacks))   )
    
    LOG_PROBAs_Combined = np.average(LOG_PROBAs, axis=0)
    
    KeyRanks_Combined = []
    for attck in range(nb_attacks):
         KeyRanks_Combined.append( [128] + [bruteforce.complexity(plaintexts, key, LOG_PROBAs_Combined[attck][i]) for i in range(len(LOG_PROBAs_Combined[attck]))] )
    np.save(data_directory+"KRs/KeyRanks_Combined_%s_%dattacks"%(dataset_name,nb_attacks), KeyRanks_Combined)

    for attck in range(nb_attacks):
        if int(args.plot_pges)!=0: 
            attacks.print_pges(key, LOG_PROBAs_Combined[attck][-1])
            attacks.print_stats(key, LOG_PROBAs_Combined[attck][-1])
        if int(args.bruteforce)!=0: bruteforce.attack(plaintexts, key, LOG_PROBAs_Combined[attck][-1], bit_bound_end=int(args.bruteforce))
